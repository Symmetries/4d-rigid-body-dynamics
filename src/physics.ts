...
function initializeState(dims : Dims) : State {
    return {
        c: new Hyperprism(
            new Vec4(0, 0, 0, 0),
            new Vec4(0, 0, 0, 0),
            new Rotor4(1, 0, 0, 0, 0, 0, 0, 0),
            new Bivec4(0, 0, 0, 0, 0, 0),
            new Vec4(1, 1, 1, 1),
        ),
    }
}

function update(state : State, dims : Dims, torque : Bivec4) {
    for (let i = 0; i < iterations; i++) {
        state.c.update(dt / iterations, torque);
    }
}

function render(state : State, dims : Dims, ctx : CanvasRenderingContext2D) {
